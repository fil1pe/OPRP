...
int main(int argc, char *argv[]) {
    ...
    while (i--) {
        j = EXECUTIONS - 1 - i;
        printf("##### Multiplicação simples (mult%d) #####\n", j);
        start_time = wtime();
    ...
    while (i--) {
        j = EXECUTIONS - 1 - i;
        printf("##### Multiplicação simples com OpenMP (mult_openmp%d) #####\n", j);
        start_time = wtime();
        thread_mmult[j] = mmultiplicar_openmp(mat_a, mat_b, nthreads);
        end_time = wtime();
        ...
        printf("  Tempo: %f s\n", end_time - start_time);
        sprintf(filename, "mult_openmp%d.result", j);
        fmat = fopen(filename, "w");
    ...
    while (i--) {
        j = EXECUTIONS - 1 - i;
        printf("##### Multiplicação em bloco com OpenMP (mult_block_openmp%d) #####\n", j);
        start_time = wtime();

        thread_mmultbloco[j] = mmultiplicar_openmp_blocos(mat_a, mat_b, nthreads);
        ...
        printf("  Tempo: %f s\n", end_time - start_time);
        sprintf(filename, "mult_block_openmp%d.result", j);
        fmat = fopen(filename, "w");
    ...
    for (int i = 0; i < EXECUTIONS; i++) {
        printf("mult%d vs. mult_openmp%d\n ˪", i, i);
    ...
    for (int i = 0; i < EXECUTIONS; i++) {
        printf("mult%d vs. mult_block_openmp%d\n ˪", i, i);
    ...
    printf(
        ...
        \nMultiplicação simples com \033[1m%d threads\033[0m (OpenMP):\t%02lf s\
        \nMultiplicação em bloco sequencial:\t\t%02lf s\
        \nMultiplicação em bloco com \033[1m%d threads\033[0m (OpenMP):\t%02lf s\n",
        media_normal_seq,
        nthreads,
        media_normal_thr,
        media_bloco_seq,
